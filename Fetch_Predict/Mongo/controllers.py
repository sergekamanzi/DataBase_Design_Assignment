...
model = tf.keras.models.load_model("/Users/nanotechnology/Desktop/FastAPI 2/Model/bank_model.h5")

# Load the scaler
with open("/Users/nanotechnology/Desktop/FastAPI 2/Model/scaler.pkl", "rb") as file:
    scaler = pickle.load(file)

# Load label encoders
with open("/Users/nanotechnology/Desktop/FastAPI 2/Model/label_encoders.pkl", "rb") as file:
...
# Function to convert np.int64 to native int
def convert_np_types(data):
    for key, value in data.items():
        if isinstance(value, np.int64):
            data[key] = int(value)  # Convert to native Python integer
    return data

async def predict_and_store(data: dict):
    ...
    input_array = np.array([data[feature] for feature in feature_order]).reshape(1, -1)
    ...
    data["deposit"] = prediction
    
    # Convert np.int64 types to native int before inserting into MongoDB
    data = convert_np_types(data)
    
    # Save to MongoDB asynchronously
    await collection.insert_one(data)
    
    return {"Deposit": "Yes" if prediction == "yes" else "No", "confidence": float(prediction_prob)}
